...
export class LocationService {
    ...
    return this.getUserCoords().pipe(
      mergeMap((coords) => {
        return this.requestCity(coords).pipe(
          map((locationUser) => {
            return locationUser.address_components[0].long_name;
          })
        );
      })
    );
  ...
  getCoordsByAddress(address: string): Observable<LatLngLiteral> {
    return this.httpClient
      .get<GeocoderResponseArray>(
      ...
      .pipe(
        map((result) => {
          // @ts-ignore
          return (result.results[0].geometry.location)as LatLngLiteral;
        })
      );
  }
